...
\begin{frame}{Why Demand Estimation?}
    ...
    \vspace{0.3cm}
    \begin{itemize}
        \item Calculate precise price elasticities: addressing price endogeneity ($p$ correlated with unobserved $\xi$). It is the starting point for any other analysis.
        \item Recover supply-side primitives: inverting the FOC allows us to recover marginal costs and markups without observing real supply-side data.
        \item Counterfactual analysis: merger simulation, introduction of new goods, or taxes.
        \item Basis for other topics: vertical relations, network effects, consumer search, and many others...
    \end{itemize}
\end{frame}
...
\begin{frame}{Why Mixed Logit \& BLP?}
    ...
    \vspace{0.3cm}
    \begin{itemize}
        \item \textbf{The IIA Restriction:}
        \begin{itemize}
            \item Logit implies proportional substitution (Red Bus / Blue Bus).
            \item Cross-price elasticities are restricted by market shares, not product similarity.
        \end{itemize}
        \vspace{0.2cm}
        \item \textbf{The Random Coefficients Solution:}
        \begin{itemize}
            \item Allows for flexible substitution patterns: Consumers substitute towards products with similar characteristics.
            \item Captures Unobserved Heterogeneity: $\beta_i = \bar{\beta} + \sigma \nu_i$. Different consumers value attributes differently.
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Learning From Replication}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item Understanding the ``Inner Loop'' (Contraction Mapping) vs. ``Outer Loop'' (GMM/MLE).
        \end{itemize}
        \vspace{0.2cm}
        \item \textbf{Computational Bottlenecks:}
        \begin{itemize}
            \item The burden of high-dimensional integration ($N \text{ consumers} \times T \text{ periods} \times R \text{ draws}$).
        \end{itemize}
        \vspace{0.2cm}
        \item \textbf{Modern Implementation:}
        \begin{itemize}
            \item \textbf{Vectorization \& Broadcasting:} Eliminating loops.
            \item \textbf{JIT Compilation (\texttt{Numba}):} If loops are necessary.
            \item \textbf{GPU Acceleration (\texttt{PyTorch}):} Parallelizing massive integral simulations.
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{The Limitation: Independence of Irrelevant Alternatives (IIA)}
    ...
    \begin{itemize}
        \item Scenario 1: Drive ($50\%$) vs. Red Bus ($50\%$). Ratio = 1:1.
        \item Scenario 2: Add a ``Blue Bus'' (identical to Red Bus).
        \item Logic Prediction (IIA): Ratio of Drive/Red Bus must stay 1:1.
        \item Result: Drive ($33\%$), Red Bus ($33\%$), Blue Bus ($33\%$).
        \item Reality: Drive ($50\%$), Red Bus ($25\%$), Blue Bus ($25\%$).
    \end{itemize}
...
\begin{frame}{Optimization Algorithms}
    ...
    \vspace{0.2cm}
    \textbf{2. Newton's Method: } Fast but expsensive to get \(H\)
...
\begin{frame}{Why PyTorch?}
    ...
    \begin{itemize}
        \item Massive parallel computation (GPU Acceleration)  (Later on this point!)
            ...
            \item GPUs have housands of small cores.
...
\begin{frame}{Implementation: Manual Derivation vs. PyTorch AutoGrad}
    \textbf{The Task:} Estimate parameters ($\theta$) using simulated maximum likelihood.
    \vspace{0.3cm}
    \begin{itemize}
        \item Numpy (Manual gradient)
        \begin{itemize}
            \item Complex math: Must derive $\frac{\partial LL}{\partial \gamma}$ manually.
            \item Runtime: approximately 1 hour 22 minutes on BlueHive.
        \end{itemize}
        \item PyTorch (AutoGrad)
        \begin{itemize}
            \item Zero math: Just write the utility function $U_{ijt}$.
            \item  Runtime: approximately 16 minutes on an RTX 4060 Ti
        \end{itemize}
    \end{itemize}
...
\begin{frame}{Transition: From Micro-Data to Market-Level Data}
    ...
    \]
    \begin{itemize}
        \item $\xi_{jt}$: Characteristics observed by consumers and firms (e.g., style, prestige), but \textit{unobserved} by the econometrician.
...
\begin{frame}{Inner Loop: Contraction Mapping}
    ...
    \vspace{-5pt}
    \begin{itemize}
        \item \textbf{Intuition:} If Model Share \(<\) Data Share, increase $\delta$.
...
\begin{frame}{Identification}
    ...
    \textbf{Types of instruments:} (Assumptions?)
    \begin{itemize}
        \item Cost shifters
        \item Hausman IV: price of product \(j\) in other markets \(p_{j't}\)
        \item BLP IV: characteristics of rival products \(x_{-j,t}\), and other variants:
        \begin{itemize}
            \item \(\sum_{j' \neq j}\left( X_{jtm} - X_{j',t,m} \right)^2\)
            \item  \(\sum_{j' \neq j}
